...
void Scoreboard::reserveRegister(unsigned wid, unsigned regnum, const active_mask_t &mask) {
  ...
  if (reg_table[wid].find(regnum) == reg_table[wid].end()) {
      ...
      if ((reg_table[wid][regnum] & mask).any()) {
          printf("Error: trying to reserve an already reserved register by same threads (sid=%d, wid=%d, regnum=%d).\n",
                 m_sid, wid, regnum);
          abort();
      }
...
void Scoreboard::releaseRegister(unsigned wid, unsigned regnum, const active_mask_t &mask) {
  if (reg_table[wid].find(regnum) == reg_table[wid].end()) return;
  
  // Bitwise AND NOT to clear only the threads that just finished
  reg_table[wid][regnum] &= ~(mask);
  
  SHADER_DPRINTF(SCOREBOARD, "Release register - warp:%d, reg: %d, mask freed: %s\n", 
                 wid, regnum, mask.to_string().c_str());
                 
  // If no threads are locking this register anymore, erase it from the map completely
  ...
  if (inst->is_load() && (inst->space.get_type() == global_space ||
    ...
    for (unsigned r = 0; r < MAX_OUTPUT_VALUES; r++) {
      if (inst->out[r] > 0) {
        ...
        if (longopregs[inst->warp_id()].find(inst->out[r]) == longopregs[inst->warp_id()].end()) {
 ...
 * DWS: Collision only occurs if the active threads of the issuing instruction 
 * overlap with the threads that currently hold the lock on that register.
 **/
bool Scoreboard::checkCollision(unsigned wid, const class inst_t* inst) const {
  ...
  const class warp_inst_t *warp_inst = static_cast<const class warp_inst_t*>(inst);
  ...
  for (it2 = inst_regs.begin(); it2 != inst_regs.end(); it2++) {
    ...
    if (reg_table[wid].find(*it2) != reg_table[wid].end()) {
      // DWS: AND the active mask of the lock with the active mask of the instruction
